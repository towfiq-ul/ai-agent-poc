...
def create_parser():
    ...
    # Parent parsers for shared arguments (used by subparsers via `parents`).
    ...
    # Global arguments on the main parser so argparse doesn't error on unknown attrs.
    parser.add_argument(
        "-p",
        "--path",
        type=str,
    ...
    subparsers.add_parser(
        "add",
        help="Add (index) documents to the database.",
