...
The response doesn't have to exactly match all words in the expected answer.
It just needs to be correct about the actual question.
...
class Evaluator(BaseEvaluator):
    def evaluate(self, query: str, response: str, expected_answer: str) -> EvaluationResult:
        ...
        if result is not None:
            is_correct = result.strip().lower() == "true"
        else:
            is_correct = False
            reasoning = f"No result tag found in response: ({response_content})"
